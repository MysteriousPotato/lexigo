...
type (
	Generator struct {
		...
		languageNames          map[language.Tag]string
...
func NewGenerator(params Params) (*Generator, error) {
	...
	if err := filepath.Walk(params.SrcPath, func(path string, info fs.FileInfo, err error) error {
		...
		if ext := filepath.Ext(info.Name()); ext == ".json" {
			...
			languageNames[tag] = strings.ReplaceAll(tag.String(), "-", "_")
			if err != nil {
				return err
			}
		}
	...
	if err := jen.
		...
		Id("package").Id(params.PkgName).Line().
		Id(importsTmpl).
		Var().
		DefsFunc(func(group *jen.Group) {
			for _, lang := range languages {
				group.Add(jen.Id(g.languageNames[lang]).Id("= language.MustParse").Call(jen.Lit(lang.String())))
			}
		}).Line().Line().
		Id("var Matcher = lexigo.NewMatcher").
		Params(
			jen.Id("[]language.Tag").ValuesFunc(func(group *jen.Group) {
				group.Add(jen.Id(g.languageNames[defaultLang]))
				for _, lang := range languages {
					if lang == defaultLang {
						continue
					}
					group.Add(jen.Id(g.languageNames[lang]))
				}
			}),
		).Line().
	...
	if err := jen.Type().Id("locale").InterfaceFunc(func(group *jen.Group) {
		for _, lang := range languages {
			group.Add(jen.Id(g.languageNames[lang]).Call().String())
		}
	}).Render(g.writer); err != nil {
		return nil, err
	}
	...
	if err := jen.Type().Id("placeholders").InterfaceFunc(func(group *jen.Group) {
		for _, lang := range languages {
			group.Add(jen.Id(g.languageNames[lang]).Call().Index().Any())
		}
	}).Render(g.writer); err != nil {
		return nil, err
	}
	...
	if err := g.writeMethod(method{
		...
		Type:        "Locale",
		Name:        "parse",
		Params:      jen.Statement{jen.Id("lang").Id("language.Tag")},
		ReturnTypes: jen.Statement{jen.String()},
		Body: jen.Statement{
			jen.Switch(jen.Id("Matcher.Match(lang)")).BlockFunc(func(group *jen.Group) {
				for _, tag := range g.languages {
					if tag == g.defaultLang {
						continue
					}

					group.Add(jen.Case(jen.Id(g.languageNames[tag])).Block(
						jen.If(jen.Id("l").Dot("placeholders").Op("==").Nil()).Block(
							jen.Return(jen.Id("l").Dot("locale").Dot(g.languageNames[tag]).Call()),
						),
						jen.Return(
							jen.Id("fmt.Sprintf").Call(
								jen.Id("l").Dot("locale").Dot(g.languageNames[tag]).Call(),
								jen.Id("l").Dot("placeholders").Dot(g.languageNames[tag]).Call().Op("..."),
							),
						),
					))
				}
				group.Add(jen.Default().Block(
					jen.If(jen.Id("l").Dot("placeholders").Op("==").Nil()).Block(
						jen.Return(jen.Id("l").Dot("locale").Dot(g.languageNames[defaultLang]).Call()),
					),
					jen.Return(
						jen.Id("fmt.Sprintf").Call(
							jen.Id("l").Dot("locale").Dot(g.languageNames[defaultLang]).Call(),
							jen.Id("l").Dot("placeholders").Dot(g.languageNames[defaultLang]).Call().Op("..."),
						),
					),
				))
			}),
	...
	if err := g.writeMethod(method{
		...
		Params: jen.Statement{
			jen.CustomFunc(jen.Options{Separator: ","}, func(group *jen.Group) {
				group.Add(jen.Id("ctx").Id("context.Context"))
			}),
		},
		ReturnTypes: jen.Statement{jen.String()},
		Body: jen.Statement{
			jen.List(jen.Id("lang"), jen.Id("_")).Op(":=").Id("lexigo.FromCtx").Call(jen.Id("ctx")),
			jen.Return(jen.Id("l").Dot("parse").Call(jen.Id("lang"))),
		},
	}); err != nil {
		return nil, err
	}
	...
	if err := g.writeMethod(method{
		...
		ReturnTypes: jen.Statement{jen.String()},
		Body: jen.Statement{
			jen.List(jen.Id("tag"), jen.Id("_")).Op(":=").Id("language.Parse").Call(jen.Id("lang")),
			jen.Return(jen.Id("l").Dot("parse").Call(jen.Id("tag"))),
		},
	}); err != nil {
		return nil, err
	}
	...
	if err := g.writeMethod(method{
		...
		Params: jen.Statement{
			jen.CustomFunc(jen.Options{Separator: ","}, func(group *jen.Group) {
				group.Add(jen.Id("lang").Id("language.Tag"))
			}),
		},
		ReturnTypes: jen.Statement{jen.String()},
		Body: jen.Statement{
...
func (g *Generator) parseLocales(ns *namespace) error {
	...
	if err != nil {
		return fmt.Errorf("invalid namespace %q for %q locale file: %w", ns.path, g.languageNames[g.defaultLang], err)
	}
...
func (g *Generator) writeLocale(ns *namespace) error {
	...
	for _, tag := range g.languages {
		...
		langSwitchCodes.Add(jen.Case(jen.Id(g.languageNames[tag])).Block(
			jen.ReturnFunc(func(group *jen.Group) {
				if placeholders == nil {
					group.Add(jen.Id("l").Dot(g.languageNames[tag]).Call())
					return
				}

				group.Add(jen.Id("fmt.Sprintf").Call(
					jen.Id("l").Dot(g.languageNames[tag]).Call(),
					jen.Id("placeholders").Dot(g.languageNames[tag]).Call().Op("..."),
				))
			}),
		))
	}
	langSwitchCodes.Add(jen.Default().Block(
		jen.ReturnFunc(func(group *jen.Group) {
			if placeholders == nil {
				group.Add(jen.Id("l").Dot(g.languageNames[g.defaultLang]).Call())
				return
			}

			group.Add(jen.Id("fmt.Sprintf").Call(
				jen.Id("l").Dot(g.languageNames[g.defaultLang]).Call(),
				jen.Id("placeholders").Dot(g.languageNames[g.defaultLang]).Call().Op("..."),
			))
		}),
	))
	...
	if err := g.writeMethod(method{
		...
		ReturnTypes: jen.Statement{jen.String()},
		Body:        jen.Statement{jen.Switch(jen.Id("Matcher.Match(lang)")).Block(langSwitchCodes...)},
	}); err != nil {
		return err
	}
